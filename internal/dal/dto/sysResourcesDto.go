...
type CreateSysResourcesDTO struct {
	Title string `json:"title" validate:"required,max=50"` // 名称:按钮标题,或菜单标题
	URL string `json:"uRL" validate:"required,url,max=100"` // 按钮请求url,或菜单路由
	Method string `json:"method" validate:"omitempty,max=50"` // 接口的请求方式
	Icon string `json:"icon" validate:"omitempty,max=100"` // 菜单小图标
	...
	ParentID int64 `json:"parentId" validate:"required,number,gte=1"` // 上一级id，0=顶级
	Sort int64 `json:"sort"` // 菜单,或按钮排序
	Status int64 `json:"status"` // 状态1是正常,2是禁用
	Description string `json:"description" validate:"omitempty,max=200"` // 描述
