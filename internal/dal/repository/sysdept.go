// Package repository SysDeptRepository
// 此文件由用户自定义，不会被代码生成器覆盖
// 如需扩展或覆盖基础方法，请在 SysDeptRepository 中定义
package repository

// SysDeptRepository SysDept仓储层
// 嵌入 defaultSysDeptRepository
type customerSysDeptRepository struct {
	*defaultSysDeptRepository
}

// SysDeptRepository ===== 用户自定义扩展方法请在 SysDeptRepository 中添加 =====
// 如需覆盖基础方法，实现相同的方法签名即可
type SysDeptRepository interface {
	iDefaultSysDeptRepository
}

var _ SysDeptRepository = (*customerSysDeptRepository)(nil)

// NewSysDeptRepository 创建SysDeptRepository实例
func NewSysDeptRepository() SysDeptRepository {
	return &customerSysDeptRepository{
		defaultSysDeptRepository: newDefaultSysDeptRepository(),
	}
}
...
// func (r *customerSysDeptRepository) FindLoginActivity(
...
//     },
//         "sys_dept.FindLoginActivity",       // fnName:表名.方法名
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysDeptRepository) CountByStatus(
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysDeptRepository) FindTopActive(
//     ctx context.Context, limit int,
// ) ([]*SysDept, error) {
//     return gormplus.SF(func() ([]*SysDept, error) {
//         var list []*SysDept
//         err := gormplus.Query[*SysDept](r.GetDB(), ctx).
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysDeptRepository) GetUserBalance(
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysDeptRepository) UpdateActiveStatus(
...
//     gormplus.SFInvalidate("sys_dept.GetUserBalance",
//         gormplus.BuildArgs("user_id", userId))
//
//     // 批量前缀失效:清掉所有列表/聚合类的自定义缓存
//     gormplus.SFInvalidatePrefixes([]string{
//         "sys_dept.FindLoginActivity",
//         "sys_dept.FindTopActive",
//         "sys_dept.CountByStatus",
//     })
//     return nil
...
// ─────────────────────────────────────────────────────────────────────────
//
// - fnName 用 "sys_dept.方法名" 格式,方便后续按表批量失效
