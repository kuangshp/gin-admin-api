// Package repository CasbinRuleRepository
// 此文件由用户自定义，不会被代码生成器覆盖
// 如需扩展或覆盖基础方法，请在 CasbinRuleRepository 中定义
package repository

// CasbinRuleRepository CasbinRule仓储层
// 嵌入 defaultCasbinRuleRepository
type customerCasbinRuleRepository struct {
	*defaultCasbinRuleRepository
}

// CasbinRuleRepository ===== 用户自定义扩展方法请在 CasbinRuleRepository 中添加 =====
// 如需覆盖基础方法，实现相同的方法签名即可
type CasbinRuleRepository interface {
	iDefaultCasbinRuleRepository
}

var _ CasbinRuleRepository = (*customerCasbinRuleRepository)(nil)

// NewCasbinRuleRepository 创建CasbinRuleRepository实例
func NewCasbinRuleRepository() CasbinRuleRepository {
	return &customerCasbinRuleRepository{
		defaultCasbinRuleRepository: newDefaultCasbinRuleRepository(),
	}
}
...
// func (r *customerCasbinRuleRepository) FindLoginActivity(
...
//     },
//         "casbin_rule.FindLoginActivity",       // fnName:表名.方法名
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerCasbinRuleRepository) CountByStatus(
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerCasbinRuleRepository) FindTopActive(
//     ctx context.Context, limit int,
// ) ([]*CasbinRule, error) {
//     return gormplus.SF(func() ([]*CasbinRule, error) {
//         var list []*CasbinRule
//         err := gormplus.Query[*CasbinRule](r.GetDB(), ctx).
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerCasbinRuleRepository) GetUserBalance(
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerCasbinRuleRepository) UpdateActiveStatus(
...
//     gormplus.SFInvalidate("casbin_rule.GetUserBalance",
//         gormplus.BuildArgs("user_id", userId))
//
//     // 批量前缀失效:清掉所有列表/聚合类的自定义缓存
//     gormplus.SFInvalidatePrefixes([]string{
//         "casbin_rule.FindLoginActivity",
//         "casbin_rule.FindTopActive",
//         "casbin_rule.CountByStatus",
//     })
//     return nil
...
// ─────────────────────────────────────────────────────────────────────────
//
// - fnName 用 "casbin_rule.方法名" 格式,方便后续按表批量失效
