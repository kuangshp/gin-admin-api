// Package repository SysAccountRepository
// 此文件由用户自定义，不会被代码生成器覆盖
// 如需扩展或覆盖基础方法，请在 SysAccountRepository 中定义
package repository

// SysAccountRepository SysAccount仓储层
// 嵌入 defaultSysAccountRepository
type customerSysAccountRepository struct {
	*defaultSysAccountRepository
}

// SysAccountRepository ===== 用户自定义扩展方法请在 SysAccountRepository 中添加 =====
// 如需覆盖基础方法，实现相同的方法签名即可
type SysAccountRepository interface {
	iDefaultSysAccountRepository
}

var _ SysAccountRepository = (*customerSysAccountRepository)(nil)

// NewSysAccountRepository 创建SysAccountRepository实例
func NewSysAccountRepository() SysAccountRepository {
	return &customerSysAccountRepository{
		defaultSysAccountRepository: newDefaultSysAccountRepository(),
	}
}
...
// func (r *customerSysAccountRepository) FindLoginActivity(
...
//     },
//         "sys_account.FindLoginActivity",       // fnName:表名.方法名
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysAccountRepository) CountByStatus(
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysAccountRepository) FindTopActive(
//     ctx context.Context, limit int,
// ) ([]*SysAccount, error) {
//     return gormplus.SF(func() ([]*SysAccount, error) {
//         var list []*SysAccount
//         err := gormplus.Query[*SysAccount](r.GetDB(), ctx).
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysAccountRepository) GetUserBalance(
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysAccountRepository) UpdateActiveStatus(
...
//     gormplus.SFInvalidate("sys_account.GetUserBalance",
//         gormplus.BuildArgs("user_id", userId))
//
//     // 批量前缀失效:清掉所有列表/聚合类的自定义缓存
//     gormplus.SFInvalidatePrefixes([]string{
//         "sys_account.FindLoginActivity",
//         "sys_account.FindTopActive",
//         "sys_account.CountByStatus",
//     })
//     return nil
...
// ─────────────────────────────────────────────────────────────────────────
//
// - fnName 用 "sys_account.方法名" 格式,方便后续按表批量失效
