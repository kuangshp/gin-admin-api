// Package repository SysResourcesRepository
// 此文件由用户自定义，不会被代码生成器覆盖
// 如需扩展或覆盖基础方法，请在 SysResourcesRepository 中定义
package repository

// SysResourcesRepository SysResources仓储层
// 嵌入 defaultSysResourcesRepository
type customerSysResourcesRepository struct {
	*defaultSysResourcesRepository
}

// SysResourcesRepository ===== 用户自定义扩展方法请在 SysResourcesRepository 中添加 =====
// 如需覆盖基础方法，实现相同的方法签名即可
type SysResourcesRepository interface {
	iDefaultSysResourcesRepository
}

var _ SysResourcesRepository = (*customerSysResourcesRepository)(nil)

// NewSysResourcesRepository 创建SysResourcesRepository实例
func NewSysResourcesRepository() SysResourcesRepository {
	return &customerSysResourcesRepository{
		defaultSysResourcesRepository: newDefaultSysResourcesRepository(),
	}
}
...
// func (r *customerSysResourcesRepository) FindLoginActivity(
...
//     },
//         "sys_resources.FindLoginActivity",       // fnName:表名.方法名
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysResourcesRepository) CountByStatus(
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysResourcesRepository) FindTopActive(
//     ctx context.Context, limit int,
// ) ([]*SysResources, error) {
//     return gormplus.SF(func() ([]*SysResources, error) {
//         var list []*SysResources
//         err := gormplus.Query[*SysResources](r.GetDB(), ctx).
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysResourcesRepository) GetUserBalance(
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysResourcesRepository) UpdateActiveStatus(
...
//     gormplus.SFInvalidate("sys_resources.GetUserBalance",
//         gormplus.BuildArgs("user_id", userId))
//
//     // 批量前缀失效:清掉所有列表/聚合类的自定义缓存
//     gormplus.SFInvalidatePrefixes([]string{
//         "sys_resources.FindLoginActivity",
//         "sys_resources.FindTopActive",
//         "sys_resources.CountByStatus",
//     })
//     return nil
...
// ─────────────────────────────────────────────────────────────────────────
//
// - fnName 用 "sys_resources.方法名" 格式,方便后续按表批量失效
