// Package repository SysRoleRepository
// 此文件由用户自定义，不会被代码生成器覆盖
// 如需扩展或覆盖基础方法，请在 SysRoleRepository 中定义
package repository

// SysRoleRepository SysRole仓储层
// 嵌入 defaultSysRoleRepository
type customerSysRoleRepository struct {
	*defaultSysRoleRepository
}

// SysRoleRepository ===== 用户自定义扩展方法请在 SysRoleRepository 中添加 =====
// 如需覆盖基础方法，实现相同的方法签名即可
type SysRoleRepository interface {
	iDefaultSysRoleRepository
}

var _ SysRoleRepository = (*customerSysRoleRepository)(nil)

// NewSysRoleRepository 创建SysRoleRepository实例
func NewSysRoleRepository() SysRoleRepository {
	return &customerSysRoleRepository{
		defaultSysRoleRepository: newDefaultSysRoleRepository(),
	}
}
...
// func (r *customerSysRoleRepository) FindLoginActivity(
...
//     },
//         "sys_role.FindLoginActivity",       // fnName:表名.方法名
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysRoleRepository) CountByStatus(
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysRoleRepository) FindTopActive(
//     ctx context.Context, limit int,
// ) ([]*SysRole, error) {
//     return gormplus.SF(func() ([]*SysRole, error) {
//         var list []*SysRole
//         err := gormplus.Query[*SysRole](r.GetDB(), ctx).
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysRoleRepository) GetUserBalance(
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysRoleRepository) UpdateActiveStatus(
...
//     gormplus.SFInvalidate("sys_role.GetUserBalance",
//         gormplus.BuildArgs("user_id", userId))
//
//     // 批量前缀失效:清掉所有列表/聚合类的自定义缓存
//     gormplus.SFInvalidatePrefixes([]string{
//         "sys_role.FindLoginActivity",
//         "sys_role.FindTopActive",
//         "sys_role.CountByStatus",
//     })
//     return nil
...
// ─────────────────────────────────────────────────────────────────────────
//
// - fnName 用 "sys_role.方法名" 格式,方便后续按表批量失效
