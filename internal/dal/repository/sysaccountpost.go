// Package repository SysAccountPostRepository
// 此文件由用户自定义，不会被代码生成器覆盖
// 如需扩展或覆盖基础方法，请在 SysAccountPostRepository 中定义
package repository

// SysAccountPostRepository SysAccountPost仓储层
// 嵌入 defaultSysAccountPostRepository
type customerSysAccountPostRepository struct {
	*defaultSysAccountPostRepository
}

// SysAccountPostRepository ===== 用户自定义扩展方法请在 SysAccountPostRepository 中添加 =====
// 如需覆盖基础方法，实现相同的方法签名即可
type SysAccountPostRepository interface {
	iDefaultSysAccountPostRepository
}

var _ SysAccountPostRepository = (*customerSysAccountPostRepository)(nil)

// NewSysAccountPostRepository 创建SysAccountPostRepository实例
func NewSysAccountPostRepository() SysAccountPostRepository {
	return &customerSysAccountPostRepository{
		defaultSysAccountPostRepository: newDefaultSysAccountPostRepository(),
	}
}
...
// func (r *customerSysAccountPostRepository) FindLoginActivity(
...
//     },
//         "sys_account_post.FindLoginActivity",       // fnName:表名.方法名
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysAccountPostRepository) CountByStatus(
...
//     },
//         "sys_account_post.CountByStatus",
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysAccountPostRepository) FindTopActive(
//     ctx context.Context, limit int,
// ) ([]*SysAccountPost, error) {
//     return gormplus.SF(func() ([]*SysAccountPost, error) {
//         var list []*SysAccountPost
//         err := gormplus.Query[*SysAccountPost](r.GetDB(), ctx).
...
//         return list, err
//     },
//         "sys_account_post.FindTopActive",
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysAccountPostRepository) GetUserBalance(
...
//     },
//         "sys_account_post.GetUserBalance",
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysAccountPostRepository) UpdateActiveStatus(
...
//     gormplus.SFInvalidate("sys_account_post.GetUserBalance",
//         gormplus.BuildArgs("user_id", userId))
//
//     // 批量前缀失效:清掉所有列表/聚合类的自定义缓存
//     gormplus.SFInvalidatePrefixes([]string{
//         "sys_account_post.FindLoginActivity",
//         "sys_account_post.FindTopActive",
//         "sys_account_post.CountByStatus",
//     })
//     return nil
...
// ─────────────────────────────────────────────────────────────────────────
//
// - fnName 用 "sys_account_post.方法名" 格式,方便后续按表批量失效
