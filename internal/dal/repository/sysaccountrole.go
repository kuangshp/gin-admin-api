// Package repository SysAccountRoleRepository
// 此文件由用户自定义，不会被代码生成器覆盖
// 如需扩展或覆盖基础方法，请在 SysAccountRoleRepository 中定义
package repository

// SysAccountRoleRepository SysAccountRole仓储层
// 嵌入 defaultSysAccountRoleRepository
type customerSysAccountRoleRepository struct {
	*defaultSysAccountRoleRepository
}

// SysAccountRoleRepository ===== 用户自定义扩展方法请在 SysAccountRoleRepository 中添加 =====
// 如需覆盖基础方法，实现相同的方法签名即可
type SysAccountRoleRepository interface {
	iDefaultSysAccountRoleRepository
}

var _ SysAccountRoleRepository = (*customerSysAccountRoleRepository)(nil)

// NewSysAccountRoleRepository 创建SysAccountRoleRepository实例
func NewSysAccountRoleRepository() SysAccountRoleRepository {
	return &customerSysAccountRoleRepository{
		defaultSysAccountRoleRepository: newDefaultSysAccountRoleRepository(),
	}
}
...
// func (r *customerSysAccountRoleRepository) FindLoginActivity(
...
//     },
//         "sys_account_role.FindLoginActivity",       // fnName:表名.方法名
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysAccountRoleRepository) CountByStatus(
...
//     },
//         "sys_account_role.CountByStatus",
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysAccountRoleRepository) FindTopActive(
//     ctx context.Context, limit int,
// ) ([]*SysAccountRole, error) {
//     return gormplus.SF(func() ([]*SysAccountRole, error) {
//         var list []*SysAccountRole
//         err := gormplus.Query[*SysAccountRole](r.GetDB(), ctx).
...
//         return list, err
//     },
//         "sys_account_role.FindTopActive",
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysAccountRoleRepository) GetUserBalance(
...
//     },
//         "sys_account_role.GetUserBalance",
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysAccountRoleRepository) UpdateActiveStatus(
...
//     gormplus.SFInvalidate("sys_account_role.GetUserBalance",
//         gormplus.BuildArgs("user_id", userId))
//
//     // 批量前缀失效:清掉所有列表/聚合类的自定义缓存
//     gormplus.SFInvalidatePrefixes([]string{
//         "sys_account_role.FindLoginActivity",
//         "sys_account_role.FindTopActive",
//         "sys_account_role.CountByStatus",
//     })
//     return nil
...
// ─────────────────────────────────────────────────────────────────────────
//
// - fnName 用 "sys_account_role.方法名" 格式,方便后续按表批量失效
