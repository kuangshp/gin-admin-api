// Package repository SysPostRepository
// 此文件由用户自定义，不会被代码生成器覆盖
// 如需扩展或覆盖基础方法，请在 SysPostRepository 中定义
package repository

// SysPostRepository SysPost仓储层
// 嵌入 defaultSysPostRepository
type customerSysPostRepository struct {
	*defaultSysPostRepository
}

// SysPostRepository ===== 用户自定义扩展方法请在 SysPostRepository 中添加 =====
// 如需覆盖基础方法，实现相同的方法签名即可
type SysPostRepository interface {
	iDefaultSysPostRepository
}

var _ SysPostRepository = (*customerSysPostRepository)(nil)

// NewSysPostRepository 创建SysPostRepository实例
func NewSysPostRepository() SysPostRepository {
	return &customerSysPostRepository{
		defaultSysPostRepository: newDefaultSysPostRepository(),
	}
}
...
// func (r *customerSysPostRepository) FindLoginActivity(
...
//     },
//         "sys_post.FindLoginActivity",       // fnName:表名.方法名
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysPostRepository) CountByStatus(
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysPostRepository) FindTopActive(
//     ctx context.Context, limit int,
// ) ([]*SysPost, error) {
//     return gormplus.SF(func() ([]*SysPost, error) {
//         var list []*SysPost
//         err := gormplus.Query[*SysPost](r.GetDB(), ctx).
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysPostRepository) GetUserBalance(
...
// ─────────────────────────────────────────────────────────────────────────
//
// func (r *customerSysPostRepository) UpdateActiveStatus(
...
//     gormplus.SFInvalidate("sys_post.GetUserBalance",
//         gormplus.BuildArgs("user_id", userId))
//
//     // 批量前缀失效:清掉所有列表/聚合类的自定义缓存
//     gormplus.SFInvalidatePrefixes([]string{
//         "sys_post.FindLoginActivity",
//         "sys_post.FindTopActive",
//         "sys_post.CountByStatus",
//     })
//     return nil
...
// ─────────────────────────────────────────────────────────────────────────
//
// - fnName 用 "sys_post.方法名" 格式,方便后续按表批量失效
